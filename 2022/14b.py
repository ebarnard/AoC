...
def run(rock_paths):
    ...
    width = max_u - min_u + 3 + 600
    origin = (min_u - 1 - 300, min_v - 1)
    ...
    while True:
        ...
        settled = False
        while not settled:
            assert sand_y + 1 < height
            assert sand_x - 1 >= 0
            assert sand_x + 1 < width

            if world[sand_x, sand_y] != EMPTY:
                break
...
if __name__ == "__main__":
    assert run(parse("14-test.txt")) == 93
    assert run(parse("14-real.txt")) == 31706
